...
module.exports = {
  submissions: {
    all: function() {
      ...
      if( cache['submissions'] ) {
        promise = Promise.resolve(cache['submissions']);
      }

      const uncachedPromise = fetch(`${baseUrl}/submissions`, {
  ...
  captions: {
    ...
    forSubmission: function(id) {
      return fetch(`${baseUrl}/submissions/${id}/captions`, {
        headers: { 'X-Device-Id': deviceId },
      }).then(function(response) {
        if( !response.ok ) { throw new Error(response.status); }
        return response.json()
  ...
  pushTokens: {
    registerIOS: function(token) {
      return fetch(`${baseUrl}/ios/pushTokens`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Device-Id': deviceId },
    ...
    registerAndroid: function(token) {
      return fetch(`${baseUrl}/android/pushTokens`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Device-Id': deviceId },
        body: JSON.stringify({token: token}),
      })
  ...
  killSwitch: function() {
    return fetch(`${baseUrl}/kill?appVersion=1.0.2`, {
      headers: { 'X-Device-Id': deviceId },
    }).then(function(response) {
      if( !response.ok ) { throw new Error(response.status); }
      return response.json();
